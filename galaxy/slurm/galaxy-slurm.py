# renamed drmaa.py from brainstorm galaxy slurm fork to slurm.py
# and made some small changes. Copy this file as  slurm.py to runners dir
# and make sure you set the slurm -p, -t and -A option in universe_wsgi.ini:
# default_cluster_job_runner = slurm://-p any -t 3-0 -A noaccount/
# if you installed python-drmaa outside of galaxy using your package manager
# the slurm runner will use that package:
# Debug output:
#  **** using python-drmaa from path /usr/lib/pymodules/python2.7/ ******
#  ******** DRMAA IMPORTED ************
#  Supported DRM systems: SLURM 2.3.5
#  Supported DRMAA implementations: PSNC DRMAA for SLURM 1.0.4
#  Version 1.0
#  ********* DRMAA INITIALIZED ************
...
import glob
g_drmaa=0
p = glob.glob('/usr/lib/pymodules/python*/')
   # we prefer the standard ubuntu python-drmaa package if it is installed
if p:	
    if os.path.isdir(p[0]+'drmaa'):
        sys.path.insert(0,p[0])
        log.debug("**** using python-drmaa from path %s ******" % p[0])
    else:
        p = glob.glob('/usr/local/lib/python*/site-packages')
	if p:
	    if os.path.isdir(p[0]+'/drmaa'):
		log.debug("**** using local python-drmaa from path %s ******" % p[0])
                sys.path.insert(0,p[0])
            else:
	        g_drmaa=1
else:
...
log.debug("********* DRMAA IMPORTED ************")
s = drmaa.Session()
s.initialize()
log.debug('Supported DRM systems: ' + str(s.drmsInfo))
log.debug('Supported DRMAA implementations: ' + str(s.drmaaImplementation))
log.debug('Version ' + str(s.version))
s.exit()
log.debug ("********* DRMAA INITIALIZED ************")
...
class DRMAAJobRunner( BaseJobRunner ):
    ...
    def queue_job( self, job_wrapper ):
        ...
        # get the native line from universe_wsgi.ini and split on whitespace
        native_array = self.get_native_spec( runner_url ).split()
        
        # make into a hash
        native_dict = {}

        for i in range(0,len(native_array),2):
            native_dict[native_array[i]] = native_array[i+1]
        
        # check if the user is even logged in
        if job_wrapper.get_job().user is not None:
            # override with web interface options if they have been set
            if job_wrapper.get_job().user.preferences.get('project', '') is not None:
                native_dict['-A'] = job_wrapper.get_job().user.preferences.get('project', '')

            if job_wrapper.get_job().user.preferences.get('time_limit', '') is not None:
                native_dict['-t'] = job_wrapper.get_job().user.preferences.get('time_limit', '')

            if job_wrapper.get_job().user.preferences.get('partition', '') is not None:
                native_dict['-p'] = job_wrapper.get_job().user.preferences.get('partition', '')

        time_limit = native_dict['-t']

        # check if time is given in days (can not be handled by slurm-drmaa), other wise convert to hours
        timelist=native_dict['-t'].partition('-')
        if timelist[0]:
           native_dict['-t'] = "%s:00:00" % (int(timelist[0])*24 + int(timelist[2]))

        jt.jobName = "GalaxyJob_%s" % (job_wrapper.get_id_tag()) # store job name, to be able to see which
        jt.hardWallclockTimeLimit = native_dict['-t']

        # remove values set manually and concatinte the rest to nativeSpecification
        del native_dict['-t']
        native_spec = ''
        for k in native_dict:
            native_spec = native_spec + k + ' ' + native_dict[k] + ' '
        jt.nativeSpecification = native_spec

        script = drm_template % (native_dict['-A'], native_dict['-p'], time_limit, job_wrapper.galaxy_lib_dir,
    ...
    def finish_job( self, drm_job_state ):
        ...
        try:
            drm_job_state.job_wrapper.finish(unicode(stdout, 'utf-8'), unicode(stderr, 'utf-8'))
        except:
